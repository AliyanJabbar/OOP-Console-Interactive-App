...
while (true) {
    ...
    if (user.select === "INTERACT WITH TEACHER") {
        let user = await inquirer.prompt({
            type: "input",
            message: chalk.bold.yellow("WRITE THE TEACHER'S NAME YOU LIKE TO INTERACT WITH..."),
            name: "data",
        });
        let teacher = person.teachers.find((val) => val === user.data.toUpperCase());
        if (!teacher) {
            person.addTeacher(user.data.toUpperCase());
            console.log(chalk.bold.green(`TEACHER ${user.data.toUpperCase()} HAS BEEN ADDED SUCCESSFULLY!!!`));
            console.log(person.teachers);
            console.log(chalk.bold.blue(`HELLO! I AM ${user.data.toUpperCase()}. NICE TO MEET YOU!`));
        }
        else {
            console.log(chalk.bold.blue(`HELLO! I AM ${user.data.toUpperCase()}. NICE TO MEET YOU!`));
        }
    ...
    if (user.select === "INTERACT WITH STUDENT") {
        let user = await inquirer.prompt({
            type: "input",
            message: chalk.bold.yellow("WRITE THE STUDENT'S NAME YOU LIKE TO INTERACT WITH..."),
            name: "data",
        });
        let student = person.students.find((val) => val === user.data.toUpperCase());
        if (!student) {
            person.addStudent(user.data.toUpperCase());
            console.log(chalk.bold.green(`STUDENT ${user.data.toUpperCase()} HAS BEEN ADDED SUCCESSFULLY!!!`));
